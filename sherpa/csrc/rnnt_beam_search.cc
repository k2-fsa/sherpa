...
std::vector<std::vector<int32_t>> FastBeamSearch(
    ...
    std::vector<std::shared_ptr<k2::rnnt_decoding::RnntDecodingStream>>
        &decode_streams_list) {
  TORCH_CHECK(encoder_out.dim() == 3, encoder_out.dim(), " vs ", 3);
  TORCH_CHECK(processed_lens.dim() == 1, processed_lens.dim(), " vs ", 1);
  TORCH_CHECK(decode_streams_list.size() == encoder_out.size(0),
              decode_streams_list.size(), " vs ", encoder_out.size(0));
  TORCH_CHECK(decode_streams_list.size() == processed_lens.numel(),
              decode_streams_list.size(), " vs ", processed_lens.numel());

  auto decode_streams = k2::rnnt_decoding::RnntDecodingStreams(
      decode_streams_list, *decode_config);
  ...
  for (int32_t t = 0; t < T; ++t) {
    ...
    k2::Array2<int32_t> contexts;
    decode_streams.GetContexts(&shape, &contexts);
    auto contexts_tensor = k2::ToTorch<int32_t>(contexts);
    // `nn.Embedding()` in torch below v1.7.1 supports only torch.int64
    contexts_tensor = contexts_tensor.to(torch::kInt64);

    auto decoder_out = model.ForwardDecoder(contexts_tensor).squeeze(1);
    decoder_out = model.ForwardDecoderProj(decoder_out);

    auto current_encoder_out = encoder_out.index({torch::indexing::Slice(), t});
    auto row_ids = k2::ToTorch<int32_t>(shape.RowIds(1));
    current_encoder_out = torch::index_select(current_encoder_out, 0, row_ids);

    auto logits = model.ForwardJoiner(current_encoder_out, decoder_out);
    auto logprobs = logits.log_softmax(-1);
    auto logprobs_array = k2::FromTorch<float>(logprobs, k2::Array2Tag{});
    decode_streams.Advance(logprobs_array);
  }

  decode_streams.TerminateAndFlushToStreams();

  k2::FsaVec ofsa;
  k2::Array1<int32_t> out_map;
  processed_lens = processed_lens.to(torch::kInt32);
  processed_lens = processed_lens.contiguous();
  std::vector<int32_t> processed_frames(
      processed_lens.data_ptr<int32_t>(),
      processed_lens.data_ptr<int32_t>() + processed_lens.numel());

  decode_streams.FormatOutput(processed_frames, &ofsa, &out_map);

  ofsa = ShortestPath(ofsa);

  auto labels_array = k2::LabelsOfFsaAsArray1(ofsa);
  k2::RaggedShape labels_shape = k2::RemoveAxis(ofsa.shape, 1);
  auto ragged_labels = k2::Ragged<int32_t>(labels_shape, labels_array);
  ragged_labels = k2::RemoveValuesLeq(ragged_labels, 0);

  return ragged_labels.ToVecVec();
}
