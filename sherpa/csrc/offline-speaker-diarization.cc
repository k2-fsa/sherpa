// sherpa/csrc/offline-speaker-diarization.cc
//
...
bool OfflineSpeakerDiarizationConfig::Validate() const {
  ...
  if (min_duration_on < 0) {
    SHERPA_LOGE("min_duration_on %.3f is negative", min_duration_on);
    return false;
  }

  if (min_duration_off < 0) {
    SHERPA_LOGE("min_duration_off %.3f is negative", min_duration_off);
    return false;
  }
...
OfflineSpeakerDiarizationResult OfflineSpeakerDiarization::Process(
  ...
  return impl_->Process(samples, std::move(callback), callback_arg);
}
