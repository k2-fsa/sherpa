...
if [ -z $SHERPA_CONDA_TOKEN ]; then
  ...
  conda build --no-test --no-anaconda-upload -c pytorch -c k2-fsa -c kaldifeat -c kaldi_native_io ./scripts/conda-cpu-windows/sherpa
else
  conda build --no-test -c pytorch -c k2-fsa -c kaldifeat -c kaldi_native_io --token $SHERPA_CONDA_TOKEN ./scripts/conda-cpu-windows/sherpa
fi
